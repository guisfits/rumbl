defmodule RumblWeb.VideoChannel do
  ...
  def join("videos:" <> video_id, params, socket) do
    ...
    annotations =
      video
      |> Multimedia.list_annotation(last_seen_id)
  ...
  defp compute_additional_info(annotation, socket) do
    for result <- InfoSys.compute(annotation.body, limit: 1, timout: 10_000) do
      ...
      case Multimedia.annotate_video(backend_user, annotation.video_id, attrs) do
        {:ok, ann} -> broadcast_annotation(socket, backend_user, ann)
        {:error, _} -> :ignore
      end
    end
  end
